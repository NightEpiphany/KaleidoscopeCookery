...
public final class TeapotOverlayEvent {
    ...
    private static void drawWordWrap(GuiGraphics graphics, Font font, Component text, int centerX, int startY) {
        int y = startY;
        for (FormattedCharSequence sequence : font.split(text, 160)) {
            graphics.drawString(font, sequence, centerX - font.width(sequence) / 2, y, -1);
            y += font.lineHeight;
        }
    }
}
