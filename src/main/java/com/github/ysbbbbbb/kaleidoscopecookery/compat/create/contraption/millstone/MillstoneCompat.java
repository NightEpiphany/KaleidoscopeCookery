package com.github.ysbbbbbb.kaleidoscopecookery.compat.create.contraption.millstone;
...
public class MillstoneCompat {
    ...
    public static void register() {
        IEvent.CALLBACK.register(event -> {
            if (event instanceof MillstoneMatchRecipeEvent.Post post)
                afterMillstoneRecipeMatch(post);
        });
    }

    @SuppressWarnings("deprecation")
    public static void afterMillstoneRecipeMatch(MillstoneMatchRecipeEvent.Post event) {
        MillstoneRecipe rawOutput = event.getRawOutput();
        // 如果机械动力的配方和本模组配方有重合，优先选择本模组的配方
        if (rawOutput.getId() != MillstoneRecipeSerializer.EMPTY_ID) {
            return;
        }
        ItemStack[] items = event.getContainer().items.toArray(ItemStack[]::new);
