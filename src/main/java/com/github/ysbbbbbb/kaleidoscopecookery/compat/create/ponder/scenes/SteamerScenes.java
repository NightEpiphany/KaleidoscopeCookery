...
public class SteamerScenes {
    public static void introduction(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.world().modifyBlock(steamerPos1, (s) -> ModBlocks.STEAMER.defaultBlockState()
        ...
        scene.overlay().showControls(vector.blockSurface(steamerPos1, Direction.UP), Pointing.DOWN, 35)
                .rightClick()
                .withItem(new ItemStack(ModItems.STEAMER));
        scene.idle(7);
        scene.world().modifyBlock(steamerPos1, (s) -> ModBlocks.STEAMER.defaultBlockState()
                .setValue(SteamerBlock.FACING, Direction.NORTH)
    ...
    public static void cooking(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        ItemStack steamer = ModItems.STEAMER.getDefaultInstance();
        TagValueOutput tag = TagValueOutput.createWithoutContext(ProblemReporter.DISCARDING);
        NonNullList<ItemStack> first = NonNullList.withSize(4, new ItemStack(ModItems.RAW_DOUGH));
        ContainerHelper.saveAllItems(tag, first);
        BlockItem.setBlockEntityData(steamer, ModBlocks.STEAMER_BE, tag);
        ...
        scene.overlay().showControls(util.vector().blockSurface(steamerPos1, Direction.UP), Pointing.DOWN, 35)
                .rightClick()
                .withItem(new ItemStack(ModItems.RAW_DOUGH));
        scene.idle(7);
        scene.world().modifyBlockEntity(steamerPos1, SteamerBlockEntity.class, (e) -> {
            ItemStack dough = new ItemStack(ModItems.RAW_DOUGH, 4);
            for (int i = 0; i < 4; i++) {
        ...
        scene.world().modifyBlock(steamerPos1, (s) -> ModBlocks.STEAMER.defaultBlockState()
                ...
                .setValue(SteamerBlock.HAS_LID, false), false);
        scene.world().modifyBlockEntity(steamerPos1, SteamerBlockEntity.class, (e) -> {
            ItemStack dough = new ItemStack(ModItems.MANTOU, 8);
            for (int i = 0; i < 8; i++) {
                e.getItems().set(i, dough.split(1));
            }
            e.setChanged();
        ...
        scene.idle(28);
        scene.overlay().showControls(util.vector().blockSurface(steamerPos1, Direction.UP), Pointing.DOWN, 20)
                .withItem(new ItemStack(ModItems.MANTOU));
        scene.idle(30);
