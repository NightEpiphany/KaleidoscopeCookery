...
public class BlockLootTables extends BlockLootSubProvider {
    ...
    public BlockLootTables(final LootTableSubProvider.Context output) {
        super(Set.of(), FeatureFlags.REGISTRY.allFlags(), output);
        this.enchantment = (HolderLookup.RegistryLookup<Enchantment>) output.lookup(Registries.ENCHANTMENT);
    }
    ...
    public void generate() {
        ...
        LootPoolEntryContainer.Builder<?> greenChili = LootItem.lootTableItem(ModItems.GREEN_CHILI)
        ...
        LootPoolEntryContainer.Builder<?> caterpillar = LootItem.lootTableItem(ModItems.CATERPILLAR)
        ...
        var countFunction = SetItemCountFunction.setCount(ContextIntProviders.between(2, 4));
        LootPool.Builder ricePanicle = LootPool.lootPool().add(AlternativesEntry.alternatives(
                LootItem.lootTableItem(ModItems.RICE_PANICLE).when(riceCropBuilder).apply(countFunction),
                LootItem.lootTableItem(riceSeed)));
        ...
        generateCustom(this.output::accept);
    ...
    private LootTable.Builder createChiliRistraLootTable() {
        ...
        LootItemCondition.Builder condition = MatchBlock.blockMatches(this.blocks, ModBlocks.CHILI_RISTRA, isSheared);

        LootItemConditionalFunction.Builder<?> normalDrop = SetItemCountFunction.setCount(ContextIntProviders.exactly(6));
        LootItemConditionalFunction.Builder<?> shearedDrop = SetItemCountFunction.setCount(ContextIntProviders.exactly(3));

        LootPoolEntryContainer.Builder<?> normalLoot = LootItem.lootTableItem(ModItems.RED_CHILI).apply(normalDrop);
        LootPoolEntryContainer.Builder<?> shearedLoot = LootItem.lootTableItem(ModItems.RED_CHILI).apply(shearedDrop);

        builder.add(AlternativesEntry.alternatives(shearedLoot.when(condition), normalLoot));
    ...
    private LootTable.Builder createStrungMushroomsLootTable() {
        ...
        LootItemCondition.Builder condition = MatchBlock.blockMatches(this.blocks, ModBlocks.STRUNG_MUSHROOMS, isSheared);

        LootItemConditionalFunction.Builder<?> normalDrop = SetItemCountFunction.setCount(ContextIntProviders.exactly(6));
        LootItemConditionalFunction.Builder<?> shearedDrop = SetItemCountFunction.setCount(ContextIntProviders.exactly(3));

        LootPoolEntryContainer.Builder<?> normalLoot = LootItem.lootTableItem(Items.BROWN_MUSHROOM).apply(normalDrop);
        LootPoolEntryContainer.Builder<?> shearedLoot = LootItem.lootTableItem(Items.BROWN_MUSHROOM).apply(shearedDrop);
    ...
    private LootItemCondition.Builder createCropBuilder(Block cropBlock) {
        ...
        return MatchBlock.blockMatches(this.blocks, cropBlock, property);
    }
    ...
    private LootItemCondition.Builder createRiceCropBuilder() {
        ...
        return MatchBlock.blockMatches(this.blocks, ModBlocks.RICE_CROP, property);
    }


    private void generateCustom(@NonNull BiConsumer<ResourceKey<LootTable>, LootTable.Builder> output) {
    ...
    private LootPoolEntryContainer.Builder<?> getSeed(ItemLike item) {
        return getSeed(item, 0.125F);
    }

    private LootPoolEntryContainer.Builder<?> getSeed(ItemLike item, float probability) {
    ...
    private void dropFoodBite(Block block, Item food, ItemLike... lootItems) {
        ...
        LootItemCondition.Builder builder = MatchBlock.blockMatches(this.blocks, foodBiteBlock, notBite);

        LootTable.Builder lootTable = LootTable.lootTable();
        for (int i = 0; i < lootItems.length; i++) {
            ItemLike itemLike = lootItems[i];
            LootPool.Builder rolls = LootPool.lootPool().setRolls(ContextIntProviders.exactly(1)).when(ExplosionCondition.survivesExplosion());
            if (i == 0) rolls.add(AlternativesEntry.alternatives(LootItem.lootTableItem(food).when(builder), LootItem.lootTableItem(itemLike)));
            else rolls.add(AlternativesEntry.alternatives(EmptyLootItem.emptyItem().when(builder), LootItem.lootTableItem(itemLike)));
    ...
    private void dropOthers(Block block, ItemLike... lootItems) {
        ...
        for (ItemLike itemLike : lootItems) {
            LootPool.Builder rolls = LootPool.lootPool().setRolls(ContextIntProviders.exactly(1))
