...
public class ExtraLootTableDrop {
    ...
    private static void addOilDrop(LootTable.Builder tableBuilder, int rolls, HolderLookup.Provider registries) {
        // 厨具刀杀猪掉油
        HolderLookup<Item> holderLookup = registries.lookupOrThrow(Registries.ITEM);
    ...
    private static void addSeedDrop(LootTable.Builder tableBuilder, HolderLookup.Provider registries) {
        ...
        HolderLookup<Item> item = registries.lookupOrThrow(Registries.ITEM);
        // 穿戴草帽掉落番茄辣椒等种子
        var tomato = getSeed(ModItems.TOMATO_SEED, enchantment, item);
        var chili = getSeed(ModItems.CHILI_SEED, enchantment, item);
        var lettuce = getSeed(ModItems.LETTUCE_SEED, enchantment, item);
        var rice = getSeed(ModItems.WILD_RICE_SEED, enchantment, item);
    ...
    private static LootPoolSingletonContainer.Builder<?> getSeed(ItemLike item, HolderLookup.RegistryLookup<Enchantment> enchantment, HolderLookup<Item> itemHolder) {
        ItemPredicate hasHat = ItemPredicate.Builder.item().of(itemHolder, TagMod.STRAW_HAT).build();
